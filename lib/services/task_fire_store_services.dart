...
class TaskFireStoreService {
  ...
  void _requestTasks() {
    ...
    var pageTasksQuery = _tasksCollectionReference
        ...
        .limitToFirst(TasksLimit);

    // #5: If we have a document start the query after it
    if (_lastDocument != null) {
      pageTasksQuery = pageTasksQuery.startAt(_lastDocument);
    }
    ...
    pageTasksQuery.onValue.listen((tasksSnapshot) {
      if (tasksSnapshot.snapshot.value.isNotEmpty) {
        var tasks = tasksSnapshot.snapshot.value
        ...
        if (currentRequestIndex == _allPagedResults.length - 1) {
          _lastDocument = tasksSnapshot.snapshot.value.last;
        }
  ...
  Future addTask(Task task) async {
    try {
      await _tasksCollectionReference.push().set(task.toMap());
    } catch (e) {
